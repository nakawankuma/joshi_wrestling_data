...
`match_vs_generator_1.html`は、試合結果のテキストから試合ごとのVSカード画像を作成するブラウザツールです。外部サーバーやPythonは使用せず、HTMLファイルをブラウザで開くだけで利用できます。

### 基本的な使用方法

1. 「元テキスト」に試合結果を貼り付けます。
2. 「解析してカードを作成」を押します。
3. 自動解析された選手名、勝敗、試合時間、決まり技などを確認・修正します。
4. 個別にPNGを保存するか、全試合をZIPで一括ダウンロードします。

### 入力例

```text
1、5★STAR GP 2026決勝トーナメント1回戦（時間無制限1本勝負）
○スターライト・キッド<BLUE STARS B 3位>（13分58秒、黒虎脚殺）HANAKO●<BLUE STARS A 2位>
※備考

選手コメント/寸評 ×7
```

- 試合は`半角数字、試合ルール`で始まる行として認識します。
- タッグ戦の選手名は全角の`＆`で区切ります。
- `○`、`◯`、`〇`を勝者、`●`、`⚫`を敗者として認識します。
- `<BLUE STARS B 3位>`または`＜BLUE STARS B 3位＞`のような補足は、選手名の次の行へ小さく表示します。
- `※`で始まる行は備考として扱います。
- `選手コメント/寸評 ×7`のようなコメント件数は自動的に除外します。

### 画像出力

- 出力サイズ、背景、文字色、アクセント色、表示項目を共通設定できます。
- 各試合は正方形のPNGとして個別保存できます。
- ZIPには、`OP`画像、試合画像、`ED`画像を名前順で並ぶ連番付きファイル名で格納します。
- `OP`と`ED`はZIPダウンロード時だけ生成され、画面上のカード一覧には表示されません。
