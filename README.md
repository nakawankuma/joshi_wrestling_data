...
- 「コーナーを追加」で「○○○の歌のコーナー」などの区切りカードを作成し、各カードの下から途中へ挿入（コーナー名は改行可能）
- 共通デザイン設定の「イベント名」を、試合・コーナー・OP・EDの全画像上部へ共通表示
- 右側のテキスト入力欄や共通イベント名からフォーカスを外すと、編集内容を情報欠落のない編集用形式で「元テキスト」へ書き戻し
- 表示設定がOFFでも、備考・決着情報・タイトル区分などの元データは保持
- 画面の並び順を保ったまま、試合カード、コーナーカード、最新の元テキストをZIPへ収録
- ZIP本体とZIP内の全ファイルは `!` でファイル名を開始し、`00` の OP、`01`〜`98` の画面順カード、`99` の ED の順に表示
...
- 2グループ表示では、勝敗記号と選手名を一組として中央配置し、記号が対戦枠からはみ出さないように描画
